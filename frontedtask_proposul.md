...
AIが「聞くべきこと」を知るためには、ドメイン（業種・業務）ごとの知識が必要。
...
│                                                                             │
│  1. パターン（何を聞くべきか）                                              │
│     └─ 会計なら: 承認フロー、外貨、月跨ぎ、取消訂正...                      │
│                                                                             │
│  2. 用語（顧客が使う言葉）                                                  │
│     └─ 会計なら: 仕訳、勘定科目、貸借、試算表...                            │
│                                                                             │
│  3. 制約（守るべきルール）                                                  │
│     └─ 会計なら: 電帳法、インボイス、J-SOX...                               │
│                                                                             │
│  4. 典型構造（よくあるデータ・処理）                                        │
│     └─ 会計なら: 勘定科目体系、仕訳明細、承認ワークフロー...                │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 方法 | 内容 | メリット | デメリット |
|------|------|----------|------------|
| A. 実プロジェクト成果物 | 上流工程/のような既存ドキュメントから抽出 | 実践的、網羅的 | プロジェクト毎に偏りあり |
| B. 法令・規格 | 電帳法、インボイス制度、J-SOXなど | 公式、必須要件が明確 | 読み解くのが大変 |
| C. 既存システム分析 | freee、マネーフォワード、SAPなど | 実用的、UI/UXも参考 | 著作権注意、表面的 |
| D. ドメインエキスパート | 経理担当者へのインタビュー | 現場の生の声、暗黙知 | 人による、時間がかかる |
| E. 実運用での学習 | 要件定義を通じて蓄積 | 実践的、継続的改善 | 最初は知識が薄い |
...
│  上流工程/ の成果物                                                         │
│       ↓ 抽出・構造化                                                       │
│  domains/accounting/knowledge.yaml                                          │
│       │                                                                     │
│       ├─ patterns: [確認すべきパターン一覧]                                 │
│       ├─ terms: [用語辞書]                                                  │
│       ├─ regulations: [電帳法, インボイス...]                               │
│       ├─ typical_screens: [仕訳入力, 試算表...]                             │
│       └─ typical_entities: [仕訳, 勘定科目...]                              │
│                                                                             │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Phase 2: 実運用で磨く                                                      │
...
│  会計で確立した構造を使って:                                                │
│                                                                             │
│  ├─ 人事給与システム → hr_knowledge.yaml                                   │
│  ├─ 在庫管理システム → inventory_knowledge.yaml                            │
│  ├─ ECサイト → ec_knowledge.yaml                                           │
│  └─ ...                                                                     │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
  ...
  - category: 承認フロー
    items:
      - id: approval_threshold
        question: "承認が必要な金額の閾値は？"
        typical_values: [10万, 50万, 100万, 500万]
        follow_ups:
          - "税込？税抜？"
          - "承認者は誰？"
          - "代理承認は？"
      ...
      - id: approval_escalation
        question: "金額が大きい場合の段階承認は？"
        typical_patterns:
          - "100万以上→課長、500万以上→部長、1000万以上→役員"

  - category: 期間処理
    items:
      - id: month_end_close
        question: "月次締め処理はありますか？"
        follow_ups:
          - "締め後の修正は？"
          - "締めのタイミングは？"
          - "誰が締める？"
...
terms:
  - term: 仕訳
    aliases: [ジャーナル, JE, journal entry]
    definition: "取引を借方・貸方に分解して記録すること"

  - term: 試算表
    aliases: [TB, trial balance]
    definition: "勘定科目ごとの残高一覧"
...
typical_screens:
  - id: journal_entry
    ...
    typical_fields:
      ...
      - {name: 借方勘定, type: account, required: true}
      - {name: 貸方勘定, type: account, required: true}
      - {name: 金額, type: money, required: true}
      - {name: 摘要, type: text, required: false}
```
...
┌─────────────────────────────────────────────────────────────────────────────┐
│                                                                             │
│  提案: 上流工程フォルダから「会計ドメイン知識」を抽出する                   │
│                                                                             │
│  入力:                                                                      │
│  ├─ 上流工程/02_要件定義/03_取引パターン洗い出し.md                         │
│  ├─ 上流工程/02_要件定義/04_仕訳ルール定義.md                               │
│  ├─ 上流工程/03_基本設計/03_画面設計_伝票入力・照会.md                      │
│  └─ その他                                                                  │
│                                                                             │
│  出力:                                                                      │
│  └─ domains/accounting/knowledge.yaml                                      │
│                                                                             │
│  これを「ドメイン知識抽出」のテンプレートとして確立                          │
│  → 他ドメインにも同じ手法を適用                                            │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
### ドメイン知識ベース
10. [ ] 上流工程フォルダから会計ドメイン知識を抽出
11. [ ] knowledge.yaml のスキーマ定義
12. [ ] 知識ベース → 質問生成ロジック設計
13. [ ] 他ドメイン展開用テンプレート作成
